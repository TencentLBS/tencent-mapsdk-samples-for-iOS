...
- (QAnnotationView *)mapView:(QMapView *)mapView viewForAnnotation:(id<QAnnotation>)annotation
{
    ...
    {
        // 设置annotation的title属性，用于显示sdk默认的InfoWindow
        QMUClusterAnnotation* clusterAnnotation = (QMUClusterAnnotation*)annotation;
        clusterAnnotation.title = [NSString stringWithFormat:@"聚和值: %lu", clusterAnnotation.items.count];
...
- (UIView *)mapView:(QMapView *)mapView customCalloutForAnnotationView:(QAnnotationView *)annotationView {
    ...
    if ([annotationView.annotation isKindOfClass:[QMUClusterAnnotation class]]) {
        UIView *view = [[UIView alloc] init];
        view.backgroundColor = [UIColor whiteColor];

        QMUClusterAnnotation *annotation = (QMUClusterAnnotation *)annotationView.annotation;
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 100, 50)];
        label.text = [NSString stringWithFormat:@"聚和值: %lu", annotation.items.count];
...
- (NSArray<TypedClustableAnnotation *> *)loadClusterData
{
    ...
    for (int i = 0; i < [lines count]; i++)
    {
        ...
        annotation.title = [NSString stringWithFormat:@"聚合数: %f", hotValue];
